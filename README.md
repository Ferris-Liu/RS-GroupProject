...
然后编辑 `.env`，填入你的 DashScope API Key。如果需要自动补全 modern movies 的海报链接，也可以填入 `TMDB_API_KEY`。
...
推荐结果页会显示当前实际使用的推荐引擎状态：

- `Ranking: KNN With Means`：说明正式的 `scikit-surprise` User-based KNN 正在使用。
- `Ranking: Fallback Popular`：说明当前 Python 环境缺少 `scikit-surprise`，系统使用高评分/高热度兜底排序，不能作为正式算法实验结果。
- `Qwen: Available / Unavailable`：说明自然语言推荐理由是否真正由 Qwen 生成。
- `Modern picks`：说明本次混入了多少部 modern cold-start 推荐。

正式展示或实验时，应确保页面显示 `Ranking: KNN With Means`，否则需要安装依赖并重启 Flask。
...
从 TMDB 自动同步 50 部现代电影，先生成预览文件，不覆盖原文件：

```bash
python scripts/sync_modern_movies.py --limit 50
```

确认没问题后覆盖 `modern_movies.csv`，并自动生成 `.bak` 备份：
...
```bash
python scripts/fill_poster_urls.py --apply
```
...
Then edit `.env` and add your DashScope API Key. If you want to auto-fill poster URLs for modern movies, you can also add `TMDB_API_KEY`.

DashScope website: https://dashscope.aliyun.com/
TMDB API key page: https://www.themoviedb.org/settings/api
...
The recommendation page displays the actual engine status being used:

- `Ranking: KNN With Means`: The official `scikit-surprise` User-based KNN is active.
- `Ranking: Fallback Popular`: `scikit-surprise` is missing in the current Python environment, so the system is using a high-rating/popularity fallback. This should not be used as the formal algorithm evaluation result.
- `Qwen: Available / Unavailable`: Indicates whether natural-language recommendation reasons are actually generated by Qwen.
- `Modern picks`: Shows how many modern cold-start recommendations were mixed in.

For formal demos or experiments, make sure the page shows `Ranking: KNN With Means`; otherwise install the dependencies and restart Flask.

### Poster URL Filling Script
...
Sync 50 modern movies from TMDB and write a preview file without overwriting the original file:

```bash
python scripts/sync_modern_movies.py --limit 50
```

After checking the preview, overwrite `modern_movies.csv` and create a `.bak` backup:
...
After checking the preview, overwrite the original file:
